...
* HL7 FHIR
   * FHIR Attribute
      * The first column reflects an existing FHIR attribute in the target FHIR version.
        *  The full path to the component or attribute is derived from the source spreadsheets by combining the datatype or resource name in the spreadsheet name (e.g., CodeableConcept in the HL7 Data Type - FHIR R4: CWE[CodeableConcept] spreadsheet with the component or attribute name in column J in the spreadsheet (e.g., value) yielindg "CodeableConcept.identifier".  Depending on the resource this is used, it would actually pick up the resource attribute.  For example, RXA-5 Administered Code is mapped in HL7 Segment - FHIR R4: RXA[Immunization] to vaccineCode in the segment spreadsheet yielding Immunization.vaccineCode.  RXA-5 uses CWE and vaccineCode uses CodeableConcept, so CWE.1 in the HL7 Data Type - FHIR R4: CWE[CodeableConcept] is mapped to value, yielding CodeableConcept.value in the data type mapping spreadsheet.  Then the full path is to Immunization.vaccineCode.value.
