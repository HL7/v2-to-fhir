// HL7 Message - FHIR E2_ OML_O21 - Sheet1.csv
...
* extension[0].extension[2].valueUri = "https://docs.google.com/spreadsheets/d/14yOK5YYW8xxMNcCHtjvLw8F7N2n7zzbReWXYQMRlFlU/edit?gid=0#gid=0"
...
* group[0].element[24].target.code = #Task[1]
* group[0].element[24].target.display = "Task[1]"
* group[0].element[24].target.comment = "Processing of each ORC segment results in the creation of a new Task resource which is linked back to the ServiceRequest created for the same ORC segment. =*= The creation of a Task resource and its inclusion in the FHIR bundle should only happen when the receiving system is responsible for fulfillment of the ServiceRequest. Other systems may receive notification of the order, but should not receive a Task Resource if they are not the intended filler system. =*= The actual value of the ORC-1 Order Control Code will yield different population of the Task resource."
* group[0].element[25].code = #OML_O21.ORDER.ORC
...
* group[0].element[25].display = "Common Order"
* group[0].element[25].target.equivalence = #equivalent
* group[0].element[25].target.code = #Provenance[3]
* group[0].element[25].target.display = "Provenance[3]"
* group[0].element[25].target.comment = "Using Provenance is the preferred way, but a local implementation may decide to add extensions to the main resource.  As the use and guidance of Provenance matures we wil revisit this later."
* group[0].element[26].code = #OML_O21.ORDER.PRT
...
* group[0].element[26].display = "Participation (for Common Order)"
* group[0].element[26].target.equivalence = #equivalent
* group[0].element[26].target.code = #PractitionerRole[3]
* group[0].element[26].target.display = "PractitionerRole[3]"
...
* group[0].element[27].code = #OML_O21.ORDER.TIMING.TQ1
...
* group[0].element[27].display = "Timing/Quantity"
...
* group[0].element[28].target.code = #ServiceRequest[1]
* group[0].element[28].target.display = "ServiceRequest[1]"
* group[0].element[28].target.comment = "Incorporate OBR content into the ServiceRequest created from the ORC segment in the same segment group."
* group[0].element[29].code = #OML_O21.ORDER.OBSERVATION_REQUEST.OBR
...
* group[0].element[29].display = "Observation Request"
* group[0].element[29].target.equivalence = #equivalent
* group[0].element[29].target.code = #Specimen[each]
* group[0].element[29].target.display = "Specimen[each]"
* group[0].element[30].code = #OML_O21.ORDER.OBSERVATION_REQUEST.NTE
...
* group[0].element[30].display = "Notes and Comments (for Detail)"
...
* group[0].element[31].target.dependsOn[0].value = "IF PRT-4.1 EQUALS \"SC\" AND PRT-4.3 EQUALS \"HL70443\""
* group[0].element[31].target.dependsOn[1].property = "Computable-FHIRPath"
* group[0].element[31].target.dependsOn[1].value = "(element(\"4.1\")=\"SC\")and(element(\"4.3\")=\"HL70443\")"
* group[0].element[32].code = #OML_O21.ORDER.OBSERVATION_REQUEST.PRT
...
* group[0].element[32].display = "Participation (for Observation Request)"
* group[0].element[32].target.equivalence = #equivalent
* group[0].element[32].target.code = #PractitionerRole[3]
* group[0].element[32].target.display = "PractitionerRole[3]"
* group[0].element[32].target.dependsOn[0].property = "Computable-ANTLR"
* group[0].element[32].target.dependsOn[0].value = "IF PRT-4.1 EQUALS \"OP\" AND PRT-4.3 EQUALS \"HL70443\""
* group[0].element[32].target.dependsOn[1].property = "Computable-FHIRPath"
* group[0].element[32].target.dependsOn[1].value = "(element(\"4.1\")=\"OP\")and(element(\"4.3\")=\"HL70443\")"
* group[0].element[33].code = #OML_O21.ORDER.OBSERVATION_REQUEST.DG1
...
* group[0].element[33].display = "Diagnosis"
* group[0].element[33].target.equivalence = #equivalent
* group[0].element[33].target.code = #Condition[1]
* group[0].element[33].target.display = "Condition[1]"
* group[0].element[33].target.comment = "Processing of the DG1 segment results in the creation of a new Condition resource which is referenced in the ServiceRequest created by the ORC segment"
...
* group[0].element[34].target.dependsOn[0].value = "IF OBX-5 LST.count LESS THAN OR EQUALS 1 OR OBX-2 IS \"NA\""
* group[0].element[35].code = #OML_O21.ORDER.OBSERVATION_REQUEST.OBSERVATION.OBX
...
* group[0].element[35].display = "Observation/Result"
* group[0].element[35].target.equivalence = #equivalent
* group[0].element[35].target.code = #Observation[1]
* group[0].element[35].target.display = "Observation[1]"
* group[0].element[35].target.dependsOn[0].property = "Computable-ANTLR"
* group[0].element[35].target.dependsOn[0].value = "IF  OBX-5 LST.count GREATER THAN 1 AND OBX-2 IS NOT \"NA\""
...
* group[0].element[36].target.code = #Device[1]
* group[0].element[36].target.display = "Device[1]"
* group[0].element[36].target.dependsOn[0].property = "Computable-ANTLR"
* group[0].element[36].target.dependsOn[0].value = "IF PRT-10 VALUED"
...
* group[0].element[37].target.code = #Observation[1]
* group[0].element[37].target.display = "Observation[1]"
* group[0].element[37].target.comment = "Incorporate PRT content into the Observation created from the OBX segment in the same segment group when it represents observation location information."
* group[0].element[37].target.dependsOn[0].property = "Computable-ANTLR"
* group[0].element[37].target.dependsOn[0].value = "IF PRT-9 VALUED OR IF PRT-14 VALUED"
* group[0].element[38].code = #OML_O21.ORDER.OBSERVATION_REQUEST.OBSERVATION.PRT
...
* group[0].element[38].display = "Participation (for OBX)"
* group[0].element[38].target.equivalence = #equivalent
* group[0].element[38].target.code = #PractitionerRole[7]
* group[0].element[38].target.display = "PractitionerRole[7]"
* group[0].element[38].target.comment = "Incorporate PRT content into the Observation created from the OBX segment in the same segment group when it represents observation practitioner information."
* group[0].element[38].target.dependsOn[0].property = "Computable-ANTLR"
* group[0].element[38].target.dependsOn[0].value = "IF PRT-5 VALUED"
* group[0].element[39].code = #OML_O21.ORDER.OBSERVATION_REQUEST.OBSERVATION.NTE
...
* group[0].element[39].display = "Notes and Comments (for Results)"
* group[0].element[39].target.equivalence = #equivalent
* group[0].element[39].target.code = #Observation[1]
* group[0].element[39].target.display = "Observation[1]"
* group[0].element[40].code = #OML_O21.ORDER.OBSERVATION_REQUEST.SPECIMEN.SPM
...
* group[0].element[40].display = "Specimen"
* group[0].element[40].target.equivalence = #equivalent
* group[0].element[40].target.code = #Specimen[1]
* group[0].element[40].target.display = "Specimen[1]"
* group[0].element[41].code = #OML_O21.ORDER.OBSERVATION_REQUEST.SPECIMEN.SPM
...
* group[0].element[41].extension[0].extension[0].valueInteger = 1
* group[0].element[41].extension[0].extension[1].url = "cardinalityMax"
* group[0].element[41].extension[0].extension[1].valueInteger = 1
* group[0].element[41].display = "Specimen"
* group[0].element[41].target.equivalence = #equivalent
* group[0].element[41].target.code = #ServiceRequest[1]
* group[0].element[41].target.display = "ServiceRequest[1]"
* group[0].element[41].target.comment = "This provides a reject reason for the specimen in context of the specific test as the specimen may still be usable for other tests."
* group[0].element[41].target.dependsOn[0].property = "Computable-ANTLR"
* group[0].element[41].target.dependsOn[0].value = "IF SPM-21 VALUED"
