// mappings\segments\HL7 Segment - FHIR R4_ OBX[Observation] - OBX.csv:11
Instance: DatatypeCEtoCodeableConcept
InstanceOf: ConceptMap
Title: "Datatype CE to CodeableConcept Map"
* description = "This ConceptMap represents a mapping from the HL7 V2 Datatype CE to the FHIR CodeableConcept. It is not yet supported."
* id = "datatype-ce-to-codeableconcept"
* url = "http://hl7.org/fhir/uv/v2mappings/datatype-ce-to-codeableconcept"
* version = "1.0"
* name = "DatatypeCEtoCodeableConcept"
* status = #active
* experimental = true
* date = "2022-02-21"
* publisher = "HL7 International, Inc"
* contact.telecom.system = #email
* contact.telecom.value = "v2-to-fhir@lists.hl7.org"
* copyright = "Copyright (c) 2020, HL7 International, Inc., All Rights Reserved."
