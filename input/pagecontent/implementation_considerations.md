...
* If instead the implementers had decided to only preserve the original HL70002 code and discard the local code (UN), they would not have created Row 10 and when the transformation engine searched the table, it would have found no matching row and not created an occurrence of CodeableConcept.coding
   * It would be up to the transformation engine to determine if the lack of a matching row warrants the logging of a warning or error - aa

<img title="CWE example" alt="CWE_Guidance_Sample" src="https://github.com/HL7/v2-to-fhir/assets/17408249/246b56a5-b4f9-4b09-8653-ebf0e57b8ed7" <figcaption>Marital Status Mappings</figcaption>>
