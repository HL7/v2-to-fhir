...
* description = "This ConceptMap represents the mapping from the HL7 V2 AL1 Segment to the FHIR AllergyIntolerance Resource representing that segment."
...
* group.element[0].target.comment = "The FHIR table has a \"required\" binding. Do we need to expand the values in the FHIR table?
...
* group.element[3].target.comment = "HL70128 is a user defined table
...
* group.element[6].target.comment = "AllergyIntolerance.reaction can repeat, presumably to capture both multiple manifestations of the reaction (rash and nausea) as well as reactions to independent exposures (last week and last month) - we may want to confirm this understanding.
